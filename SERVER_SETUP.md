...
wget https://us.download.nvidia.com/XFree86/Linux-x86\_64/\<VERSION\>/NVIDIA-Linux-x86\_64-\<VERSION\>.run

chmod \+x NVIDIA-Linux-x86\_64-\<VERSION\>.run

./NVIDIA-Linux-x86\_64-\<VERSION\>.run \--dkms
...
lsmod | grep nvidia   \# check if nvidia\_uvm is missing

modprobe nvidia\_uvm   \# load it manually

\# Make it permanent:

echo "nvidia\_uvm" \>\> /etc/modules-load.d/nvidia.conf
...
mknod \-m 666 /dev/nvidia-uvm c \<UVM\_MAJOR\> 0 2\>/dev/null || true

mknod \-m 666 /dev/nvidia-uvm-tools c \<UVM\_MAJOR\> 1 2\>/dev/null || true

mkdir \-p /dev/nvidia-caps

mknod \-m 444 /dev/nvidia-caps/nvidia-cap1 c \<CAPS\_MAJOR\> 1 2\>/dev/null || true

mknod \-m 444 /dev/nvidia-caps/nvidia-cap2 c \<CAPS\_MAJOR\> 2 2\>/dev/null || true

exit 0

> ⚠️ **Major device numbers for `nvidia-uvm` and `nvidia-caps` are dynamically assigned and can change after any driver reinstall or kernel upgrade.** Always check the current numbers before trusting this script:  
>   
...
lxc.cgroup2.devices.allow: c \<UVM\_MAJOR\>:\* rwm

lxc.cgroup2.devices.allow: c \<CAPS\_MAJOR\>:\* rwm
...
./NVIDIA-Linux-x86\_64-\<VERSION\>.run \--no-kernel-module
...
ExecStart=/opt/kokoro-venv/bin/uvicorn tts\_server:app \--host 0.0.0.0 \--port 8880 \--ssl-keyfile /root/key.pem \--ssl-certfile /root/cert.pem
...
Environment="OLLAMA\_KEEP\_ALIVE=30m"
...
rm \-rf /root/rag-server/chroma\_db
...
1. **HTTPS** — reuse the same self-signed cert/key already generated for `tts_server.py` (same host, same cert is fine):  
   ...
   ExecStart=/root/rag-server/.venv/bin/uvicorn rag\_server:app \--host 0.0.0.0 \--port 8882 \--ssl-keyfile /root/key.pem \--ssl-certfile /root/cert.pem  
...
| Container's `nvidia-smi` fails after a driver reinstall/kernel upgrade | Major device numbers shifted | `cat /proc/devices | grep nvidia`, update LXC config \+ device script to match |
